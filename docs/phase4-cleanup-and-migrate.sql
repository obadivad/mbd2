-- =====================================================
-- MINISTÉRIO DO BLOCO - PHASE 4 CLEANUP & MIGRATION
-- Clean up partial tables and run complete migration
-- Date: December 2024
-- =====================================================

-- 🚨 IMPORTANT: This will clean up partial user tables and create the complete system

BEGIN;

-- =====================================================
-- 1. CLEANUP EXISTING PARTIAL TABLES
-- =====================================================

DO $$
BEGIN
    RAISE NOTICE 'Phase 4.0: Cleaning up partial user tables...';
END $$;

-- Drop existing user_locations table (incomplete from previous attempt)
DROP TABLE IF EXISTS user_locations CASCADE;

-- Drop any other partial user tables that might exist
DROP TABLE IF EXISTS user_bloco_follows CASCADE;
DROP TABLE IF EXISTS user_saved_events CASCADE;
DROP TABLE IF EXISTS user_connections CASCADE;
DROP TABLE IF EXISTS user_location_sessions CASCADE;
...
-- Drop any existing user-related indexes
DROP INDEX IF EXISTS idx_user_profiles_display_name;
DROP INDEX IF EXISTS idx_user_profiles_preferred_cities;
DROP INDEX IF EXISTS idx_user_profiles_last_active;
DROP INDEX IF EXISTS idx_user_bloco_follows_user_id;
DROP INDEX IF EXISTS idx_user_bloco_follows_bloco_id;
DROP INDEX IF EXISTS idx_user_bloco_follows_followed_at;
DROP INDEX IF EXISTS idx_user_saved_events_user_id;
DROP INDEX IF EXISTS idx_user_saved_events_event_id;
DROP INDEX IF EXISTS idx_user_saved_events_saved_at;
DROP INDEX IF EXISTS idx_user_connections_follower;
DROP INDEX IF EXISTS idx_user_connections_following;
DROP INDEX IF EXISTS idx_user_location_sessions_user_id;
DROP INDEX IF EXISTS idx_user_location_sessions_active;
DROP INDEX IF EXISTS idx_user_locations_session_id;
DROP INDEX IF EXISTS idx_chat_rooms_type_entity;
DROP INDEX IF EXISTS idx_chat_participants_user_id;
DROP INDEX IF EXISTS idx_chat_messages_room_created;
DROP INDEX IF EXISTS idx_chat_messages_user_id;
DROP INDEX IF EXISTS idx_user_activities_user_occurred;
DROP INDEX IF EXISTS idx_user_activities_type;
DROP INDEX IF EXISTS idx_user_achievements_user_id;

-- Drop any existing functions
DROP FUNCTION IF EXISTS update_bloco_followers_count() CASCADE;
DROP FUNCTION IF EXISTS update_saved_events_count() CASCADE;
DROP FUNCTION IF EXISTS update_user_connections_count() CASCADE;
DROP FUNCTION IF EXISTS update_user_last_active() CASCADE;

DO $$
BEGIN
    RAISE NOTICE 'Cleanup complete - ready for fresh migration';
END $$;
...
-- =====================================================
-- 5. USER CONNECTIONS & SOCIAL FEATURES
-- =====================================================
...
CREATE INDEX idx_user_profiles_preferred_cities ON user_profiles USING GIN(preferred_cities);
CREATE INDEX idx_user_profiles_last_active ON user_profiles(last_active_at DESC);

-- User follows indexes
CREATE INDEX idx_user_bloco_follows_user_id ON user_bloco_follows(user_id);
CREATE INDEX idx_user_bloco_follows_bloco_id ON user_bloco_follows(bloco_id);
CREATE INDEX idx_user_bloco_follows_followed_at ON user_bloco_follows(followed_at DESC);
...
CREATE INDEX idx_user_saved_events_saved_at ON user_saved_events(saved_at DESC);
...
CREATE INDEX idx_user_location_sessions_user_id ON user_location_sessions(user_id);
CREATE INDEX idx_user_location_sessions_active ON user_location_sessions(is_active, expires_at);
CREATE INDEX idx_user_locations_session_id ON user_locations(session_id, recorded_at DESC);

-- Chat indexes
CREATE INDEX idx_chat_rooms_type_entity ON chat_rooms(room_type, event_id, bloco_id);
CREATE INDEX idx_chat_participants_user_id ON chat_participants(user_id);
CREATE INDEX idx_chat_messages_room_created ON chat_messages(room_id, created_at DESC);
CREATE INDEX idx_chat_messages_user_id ON chat_messages(user_id);

-- Activity indexes
CREATE INDEX idx_user_activities_user_occurred ON user_activities(user_id, occurred_at DESC);
...
/*
📝 PHASE 4 CLEANUP & MIGRATION NOTES:

🧹 Cleanup Applied:
- Dropped existing incomplete user_locations table
- Removed any partial user tables from previous attempts
- Cleaned up conflicting indexes and functions
