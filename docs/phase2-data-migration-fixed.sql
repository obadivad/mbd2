-- =====================================================
-- MINISTÉRIO DO BLOCO - PHASE 2 MIGRATION (FIXED)
...
-- FIXED VERSION: Uses correct column names from actual events table

BEGIN;

-- =====================================================
-- 1. MIGRATE EVENTS DATA (CORRECTED)
-- =====================================================
...
BEGIN
    ...
    IF old_events_count > 0 THEN
        ...
        -- Using actual column names: title, description, start_date, end_date, location
        ...
        SELECT 
            ...
            e.bloco_id,
            e.title,
            'other'::event_type as event_type, -- Default since no type field exists
            ...
            e.created_at,
            COALESCE(e.created_at, NOW()) as updated_at
        FROM events e
...
SET slug = LOWER(
    REGEXP_REPLACE(
        REGEXP_REPLACE(
            TRANSLATE(name, 'áàâãäåāăąčçćđďéèêëēĕėęěğģīĭįıķļľłńņňñōŏőōõöøřŕşșšťţūŭůűųýÿźżžþæœß', 'aaaaaaaaacccddeeeeeeeeeggiiiikllllnnnnooooooorrssstttuuuuuyyzzzbao'), 
            '[^a-zA-Z0-9\s-]', '', 'g'
        ), 
        '\s+', '-', 'g'
    )
...
/*
📝 PHASE 2 MIGRATION NOTES (FIXED VERSION):

✅ What was completed:
- Migrated events from old to new structure (using correct column names)
...
🔧 Fixed Issues:
- Used correct column names from events table:
  - title (not name)
  - description (exists)
  - start_date (not start_time)
  - end_date (not end_time)
  - location (exists)
